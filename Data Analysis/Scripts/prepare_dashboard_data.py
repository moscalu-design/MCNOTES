...
from analysis_config import ANALYSIS_DATE, DATA_DIR, EXPORTS, SOURCE_SHEET, SOURCE_WORKBOOK
...
def records_from_csv(path: Path, columns: list[str] | None = None) -> list[dict]:
    ...
    if columns:
        df = df[columns]
    for col in ["Validation Date", "Validation Month", "BO Validation Date", "BO Validation Month"]:
...
def load_tests() -> dict:
    path = EXPORTS / "dashboard_tests.json"
    if not path.exists():
        return {"tests": []}
...
def main() -> None:
    cleaned = pd.read_csv(EXPORTS / "cleaned_database.csv")
    ...
    cleaned["Has Missing BO Date"] = cleaned["BO Validation Date"].isna()
    ...
    payload = {
        "meta": {
            ...
            "products": sorted(cleaned["Financing Product Name"].dropna().unique().tolist()),
            "departments": [
                "OPS",
                "GLO",
                "PJ",
        ...
        "records": rows,
        "templateOverview": records_from_csv(EXPORTS / "template_overview.csv"),
        "monthlyTimeSeries": records_from_csv(EXPORTS / "monthly_time_series.csv"),
        "departmentByTemplate": records_from_csv(EXPORTS / "department_word_counts_by_template.csv"),
        "batchOverview": records_from_csv(EXPORTS / "batch_folder_overview.csv"),
        "dataQuality": records_from_csv(EXPORTS / "data_quality_flags.csv"),
        "financingProducts": records_from_csv(EXPORTS / "financing_product_overview.csv"),
        "boTeamOpinions": records_from_csv(EXPORTS / "bo_team_opinion_summary.csv"),
        "boValidationSummary": records_from_csv(EXPORTS / "bo_validation_date_summary.csv"),
